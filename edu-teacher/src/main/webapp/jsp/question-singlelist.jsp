...
<table class="easyui-datagrid" id="questionSingleList" title="题库列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/question/singlelist',method:'get',pageSize:30,toolbar:toolbar">
    <thead>
        <tr>
...
<div id="questionSingleEditWindow" class="easyui-window" title="编辑" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/question/toeditsingle'" style="width:80%;height:80%;padding:10px;">
</div>
<script>

    function getSelectionsIds(){
    	var itemList = $("#questionSingleList");
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	$("#questionSingleEditWindow").window({
        		onLoad :function(){
        			//回显数据
        			var data = $("#questionSingleList").datagrid("getSelections")[0];
        			$("#questionSingleEditForm").form("load",data);
        		}
        	}).window("open");
        }
    },{
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的题库吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
               	$.post("/question/deletesingle",params, function(data){
          			if(data.state == 200){
