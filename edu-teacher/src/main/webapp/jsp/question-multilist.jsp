...
<table class="easyui-datagrid" id="questionMultiList" title="题库列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/question/multilist',method:'get',pageSize:30,toolbar:toolbar">
    <thead>
        <tr>
...
<div id="questionMultiEditWindow" class="easyui-window" title="编辑" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/question/toeditmulti'" style="width:80%;height:80%;padding:10px;">
</div>
<script>
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	$("#questionMultiEditWindow").window({
        		onLoad :function(){
        			...
        			$("#questionMultiEditForm").form("load",data);
        			if(answer != "" && answer.length > 0)
      			{
      				var answers = answer.split(",");
      				for(var i=0;i<answers.length;i++)
      				{
      					$("#questionMultiEditForm [name=answer-multi]").each(function(){ 
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的题库吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
               	$.post("/question/deletemulti",params, function(data){
          			if(data.state == 200){
